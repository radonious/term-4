...
public class Controller implements Initializable {
    ...
    @FXML private CheckBox toggleWorkerAICheckBox;
    @FXML private CheckBox toggleDroneAICheckBox;
    ...
    @FXML private Slider deathTimeWorker;
    @FXML private Slider deathTimeDrone;
    ...
    @FXML private void showCurrentObjects() {
    ...
    private void moveAlive() {
        if (simulationTime % habitat.getN() == 0) {
            Random r = new Random();
            habitat.setAngleN((float) (r.nextFloat(360) * Math.PI / 180));
        }
        boolean DroneAI = toggleDroneAICheckBox.isSelected();
        boolean WorkerAI = toggleWorkerAICheckBox.isSelected();
        for (Bee bee : habitat.getList()) {
            if (bee.getClass() == Drone.class && DroneAI) {
                ((Drone)bee).move(habitat.getV(), habitat.getAngleN(), viewPane.getWidth() - 50, viewPane.getHeight() - 50);
            } else if (bee.getClass() == Worker.class && WorkerAI) {
                ((Worker)bee).move(habitat.getV());
            }
        }
    }
    ...
    private void clearDead() {
        ...
        for (Bee bee : deads) {
            ...
            viewPane.getChildren().remove(bee.getView());
